...
function SocialLoginButton({ icon, children, onClick }: SocialLoginButtonType) {
  return (
    <button
      className="w-10/12 flex items-center gap-2 px-4 py-2 rounded-full justify-center mx-auto bg-violet-500 text-white"
      onClick={onClick}
    >
      {icon}
      {children}
...
export default function LoginPage() {
  const { loading, loginWithGitHub, loginWithTwitter } = useAuth();
